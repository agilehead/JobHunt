...
def sendOneWayMail(sender, recipients, subject, html_body, attachments=None, images=None, text_message=None):
    sendMail(sender, recipients, subject, html_body, attachments, images, text_message, '"Jobhunt.in"<mailman@jobhunt.in>')
...
def logSentMail(recipients, subject):
    with open('/apps/jobhuntin/backslash/logs/sentmail.txt', 'a') as file:
        file.write(str(datetime.datetime.utcnow()) + '\t' + str(recipients) + '\t' + subject + '\n')
    
def logError(err):
    with open('/apps/jobhuntin/backslash/logs/mailman_errors.txt', 'a') as file:
